...
namespace PhantombiteAutoTransfer.Modules
{
    ...
    /// </summary>
    public class AutoTransferMain : IModule
    {
        public string ModuleName => "AutoTransfer_Main";

        private AutoTransferCommand _logger;
        private const string MODULE = "AutoTransfer_Main";

        public void SetLogger(AutoTransferCommand logger) { _logger = logger; }
        ...
        {
            "AutotransZoneContainer",
            "AutotransLargeBlockLargeContainer",
            "AutotransLargeBlockLargeIndustrialContainer"
        };

        private static readonly HashSet<string> CONNECTOR_SUBTYPES = new HashSet<string>
        {
            "AutotransConnector",
            "AutotransConnectorSmall",
            "AutotransConnectorMedium",
            "AutotransLargeBlockInsetConnector",
            "AutotransLargeBlockInsetConnectorSmall",
            "AutotransSmallBlockInsetConnector",
            "AutotransSmallBlockInsetConnectorMedium"
        };

        private static readonly HashSet<string> LCD_SUBTYPES = new HashSet<string>
        {
            "AutotransSmallTextPanel",
            "AutotransSmallLCDPanel",
            "AutotransSmallLCDPanelWide",
            "AutotransLargeTextPanel",
            "AutotransLargeLCDPanel",
            "AutotransLargeLCDPanelWide",
            "AutotransLargeLCDPanel3x3",
            "AutotransLargeLCDPanel5x3",
            "AutotransLargeLCDPanel5x5",
            "AutotransLargeBlockCorner_LCD_1",
            "AutotransLCDLargeBlockCorner_LCD_2",
            "AutotransLargeBlockCorner_LCD_Flat_1",
            "AutotransLargeBlockCorner_LCD_Flat_2",
            "AutotransSmallBlockCorner_LCD_1",
            "AutotransSmallBlockCorner_LCD_2",
            "AutotransSmallBlockCorner_LCD_Flat_1",
            "AutotransSmallBlockCorner_LCD_Flat_2",
            "AutotransLargeCurvedLCDPanel",
            "AutotransSmallCurvedLCDPanel",
            "AutotransLargeDiagonalLCDPanel",
            "AutotransSmallDiagonalLCDPanel",
            "AutotransLargeFullBlockLCDPanel",
            "AutotransSmallFullBlockLCDPanel",
            "AutotransTransparentLCDLarge",
            "AutotransTransparentLCDSmall",
            "AutotransHoloLCDLarge",
            "AutotransHoloLCDSmall",
            "AutotransSmallBlockConsoleModuleScreens"
        };

        private static readonly HashSet<string> SORTER_SUBTYPES = new HashSet<string>
        {
            "AutotransLargeBlockConveyorSorter",
            "AutotransMediumBlockConveyorSorter",
            "AutotransSmallBlockConveyorSorter"
        };
        ...
        private const string TEMPLATE_CONTAINER =
            ...
            "\r\n" +
            "[ZoneBlocks]\r\n" +
            "# Wird automatisch befüllt nach !pbc autotrans scan\r\n" +
            "Connectors=\r\n" +
            "LCDs=\r\n" +
            "SortersIn=\r\n" +
            "SortersOut=\r\n";
        ...
        private const string TEMPLATE_LCD_GROUP =
            "[AutoTransfer]\r\n" +
            "# Gruppen-LCD: zeigt nur belegte Zonen aus der Liste, wechselt alle 30 Sek\r\n" +
            "ZoneNumber=0\r\n" +
            "LCDMode=Group\r\n" +
            "Zones=1,2,3\r\n";
        ...
        private const int                 GROUP_ZONE_DURATION   = 30; // Sekunden Zone-Anzeige auf Gruppen-LCD
        private const int                 GROUP_MAIN_DURATION   = 10; // Sekunden Übersicht auf Gruppen-LCD
        ...
        // Gruppen-LCDs: EntityId → konfigurierte ZoneNummern
        private Dictionary<long, List<int>> _groupLcdZones   = new Dictionary<long, List<int>>();
        // Gruppen-LCDs: EntityId → aktueller Rotations-Counter
        private Dictionary<long, int>       _groupLcdCounter = new Dictionary<long, int>();
        ...
        public AutoTransferMain(AutoTransferCommand commandModule, AutoTransferFileManager fileManager)
        {
        ...
        public void Init()
        {
            ...
            try
            {
                if (MyAPIGateway.Multiplayer.IsServer)
                {
                    ...
                    // HINWEIS: ScanAndDeployTemplates und LoadZonesFromCustomData werden
                    // im ersten Update ausgeführt — Entities sind in Init noch nicht bereit!
            ...
            {
                _logger?.Error(MODULE, "Fehler in Init: " + ex.Message);
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in Init:\n{ex}");
            }
        }
        ...
        public void Update()
        {
            ...
            try
            {
                // Templates schreiben beim ersten Update — Entities sind jetzt bereit
                if (!_templatesDeployed)
                {
                    ScanAndDeployTemplates();
                    _templatesDeployed = true;
                }
                ...
                if (!_zonesLoaded)
                {
                    ...
                    // Wenn keine Zonen geladen → automatisch Scan ausführen
            ...
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in Update:\n{ex}");
            }
        }
        ...
        public void Close()
        {
            ...
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in Close:\n{ex}");
            }
        ...
        public string HandleCommand(IMyPlayer player, string[] args, ulong steamId = 0)
        {
            try
            {
                if (args.Length == 0)
                    return "Usage: !pbc autotrans <in|out> <player|ship> | stop | scan | report";
                ...
                switch (cmd)
                {
                    case "help":
                        ...
                        if (_commandModule.IsAdmin(player))
                        {
                            ...
                            _commandModule.SendMessage(player, "  log on/off — [Admin] Transfer-Log speichern");
                        }
                        return "AutoTransfer Help angezeigt.";
                    ...
                    case "stop":
                        result = ExecuteSetMode(player, TransferMode.None);
                        return result ?? "Fehler: Nicht an einer Ladezone angedockt.";

                    case "scan":
                        if (!_commandModule.IsAdmin(player))
                            return "Fehler: Nur Admins können scan ausführen.";
                        ExecuteZoneScan(player);
                        return "AutoTransfer: Scan abgeschlossen.";

                    case "report":
                        if (!_commandModule.IsAdmin(player))
                            return "Fehler: Nur Admins können report ausführen.";
                        ExecuteReport(player);
                        return "AutoTransfer: Report gesendet.";

                    case "log":
                        if (!_commandModule.IsAdmin(player))
                            return "Fehler: Nur Admins können den Log-Modus ändern.";
                        if (args.Length < 2)
                        {
                            _commandModule.SendMessage(player, $"Log-Modus: {(_logMode ? "ON" : "OFF")}  |  Usage: !pbc autotrans log <on|off>");
                            return "AutoTransfer: Log-Status angezeigt.";
                        }
                        switch (sub)
                        {
                            case "on":
                                _logMode = true;
                                return "Transfer-Log: ON — jede Session wird gespeichert.";
                            case "off":
                                _logMode = false;
                                return "Transfer-Log: OFF — nur bei Fehlern speichern.";
                            default:
                                return "Usage: !pbc autotrans log <on|off>";
                        }

                    default:
                        return $"Unbekannter Command: '{cmd}' | Usage: !pbc autotrans <in|out> <player|ship>";
                }
            }
            catch (Exception ex)
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in HandleCommand:\n{ex}");
                return "Interner Fehler in AutoTransfer.";
            }
        }
        ...
        private void SetSortersForMode(ZoneData zone, TransferMode mode)
        {
            ...
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in SetSortersForMode Zone {zone.ZoneNumber}:\n{ex}");
            }
        }
        ...
        private void RunTransfers()
        {
            foreach (var kv in _states)
            {
                ...
                {
                    MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in RunTransfers Zone {state.ZoneNumber}:\n{ex}");
                }
            }
        }
        ...
        private IMyInventory GetContainerInventory(ZoneData zone)
        {
            ...
            if (block == null || block.Closed)
            {
                ...
                _logger?.Warn(MODULE, $"Container EntityId {zone.ContainerEntityId} nicht gefunden oder geschlossen.");
                return null;
            }
            var inv = block.GetInventory(0);
            if (inv == null)
                
                _logger?.Warn(MODULE, $"Container {block.DisplayName} hat kein Inventar.");
            return inv;
        }
        ...
        private void LoadKeepList()
        {
            try
            {
                ...
                string content = _fileManager.GetConfigRaw();
            ...
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in LoadKeepList:\n{ex}");
            }
        }
        ...
        private void CheckConnections()
        {
            ...
            foreach (var kv in _zones)
            {
                ...
                {
                    MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in CheckConnections Zone {kv.Key}:\n{ex}");
                }
            }
        }
        ...
        private void OnPlayerDocked(ZoneData zone, ZoneState state, ulong playerId, string playerName)
        {
            ...
            SendToPlayer(playerId, $"Ladezone {zone.ZoneNumber} reserviert.");
            SendToPlayer(playerId, "  !pbc autotrans in/out player  — Player <-> Ladezone");
        ...
        private Dictionary<string, float> TakeContainerSnapshot(ZoneData zone)
        {
            ...
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in TakeContainerSnapshot:\n{ex}");
                return null;
            }
        }
        ...
        private void SaveSessionLog(ZoneData zone, ZoneState state)
        {
            try
            {
                ...
                using (var writer = MyAPIGateway.Utilities.WriteFileInWorldStorage(fileName, typeof(AutoTransferMain)))
                {
            ...
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in SaveSessionLog:\n{ex}");
            }
        }
        ...
        private void UpdateLCDs()
        {
            ...
            foreach (var kv in _zones)
            {
                try
                {
                    ...
                    foreach (long lcdId in zone.LcdIds)
                    {
                        ...
                        {
                            MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in UpdateLCDs LCD {lcdId}:\n{ex}");
                        }
                    }
                }
                catch (Exception ex)
                {
                    MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in UpdateLCDs Zone {kv.Key}:\n{ex}");
                }
            }
            ...
            // Gruppen-LCDs — konfigurierbare Zone-Auswahl mit Rotation
            UpdateGroupLCDs();
        }
        ...
        private void UpdateMainLCDs()
        {
            ...
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in UpdateMainLCDs:\n{ex}");
            }
        }

        private void UpdateGroupLCDs()
        {
            if (_groupLcdZones.Count == 0) return;

            // Zyklus pro belegter Zone: GROUP_MAIN_DURATION + GROUP_ZONE_DURATION Sekunden
            // Keine Zone belegt → dauerhaft Group-Main
            int blockSize = GROUP_MAIN_DURATION + GROUP_ZONE_DURATION; // 40s pro Zone-Block

            foreach (var kv in _groupLcdZones)
            {
                try
                {
                    long lcdId         = kv.Key;
                    List<int> allZones = kv.Value;

                    // Nur belegte Zonen aus der konfigurierten Liste
                    var occupiedZones = new List<int>();
                    foreach (int zn in allZones)
                    {
                        ZoneState st;
                        if (_states.TryGetValue(zn, out st) && st.OccupiedByPlayerId != 0)
                            occupiedZones.Add(zn);
                    }
                    ...
                    {
                        // Keine Zone belegt → dauerhaft Übersicht, Counter zurücksetzen
                        lcd.WriteText(BuildGroupMainText(allZones), false);
                        _groupLcdCounter[lcdId] = 0;
                        continue;
                    }

                    // Counter hochzählen
                    int counter = _groupLcdCounter.ContainsKey(lcdId) ? _groupLcdCounter[lcdId] : 0;
                    counter++;
                    int totalCycle = blockSize * occupiedZones.Count;
                    if (counter >= totalCycle) counter = 0;
                    _groupLcdCounter[lcdId] = counter;

                    // Welcher Block im Zyklus? (0..N-1)
                    int blockIdx    = counter / blockSize;
                    int posInBlock  = counter % blockSize;

                    // Erste GROUP_MAIN_DURATION Sekunden des Blocks → Übersicht
                    if (posInBlock < GROUP_MAIN_DURATION)
                    {
                        lcd.WriteText(BuildGroupMainText(allZones), false);
                    }
                    else
                    {
                        // Rest des Blocks → Zone anzeigen
                        lcd.WriteText(BuildGroupZoneText(occupiedZones[blockIdx % occupiedZones.Count]), false);
                    }
                }
                catch (Exception ex)
                {
                    MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in UpdateGroupLCDs {kv.Key}:\n{ex}");
                }
            }
        }

        private string BuildGroupMainText(List<int> zoneNumbers)
        {
            _sbLcd.Clear();
            _sbLcd.AppendLine("=== LADEZONEN UEBERSICHT ===");

            foreach (int zn in zoneNumbers)
            {
                ZoneState st;
                if (!_states.TryGetValue(zn, out st) || st.OccupiedByPlayerId == 0)
                {
                    _sbLcd.AppendLine($"Zone {zn}: FREI");
                }
                else
                {
        ...
        private string BuildGroupZoneText(int zoneNumber)
        {
            _sbLcd.Clear();
            ...
            {
                _sbLcd.AppendLine($"Zone {zoneNumber}: nicht gefunden.");
                return _sbLcd.ToString();
            }

            ZoneState state;
            _states.TryGetValue(zoneNumber, out state);

            _sbLcd.AppendLine($"=== LADEZONE {zoneNumber} ===");
            ...
            if (inv != null)
            {
                float liter = (float)inv.CurrentVolume * 1000f;
                _sbLcd.AppendLine($"Container: {liter:F0}L");
                ...
                {
                    _sbLcd.AppendLine("--- Inhalt ---");
                    foreach (var item in items)
                        _sbLcd.AppendLine($"{item.Type.SubtypeId,-22} x{(int)(float)item.Amount}");
                }
            }
        ...
        /// <summary>
        /// Liest Zones=1,2,3 aus CustomData und gibt eine Liste von ZoneNummern zurück.
        /// </summary>
        private List<int> ParseGroupZones(string customData)
        {
            ...
            foreach (var rawLine in customData.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                if (trimmed.StartsWith("[") && trimmed.EndsWith("]"))
                {
                    inSection = (trimmed == "[AutoTransfer]");
                    continue;
                }
                if (!inSection) continue;
                ...
                if (trimmed.Substring(0, eq).Trim() == "Zones")
                {
                    string val = trimmed.Substring(eq + 1).Trim();
                    foreach (var part in val.Split(','))
                    {
                        int zn;
                        if (int.TryParse(part.Trim(), out zn) && zn > 0)
                            result.Add(zn);
                    }
                    break;
                }
        ...
        private void ExecuteList(IMyPlayer player, string[] args)
        {
            try
            {
                // Nur eigene Zone — funktioniert nur wenn angedockt
                ZoneState playerState = FindPlayerState(player?.SteamUserId ?? 0);
                if (playerState == null)
                {
                ...
                int zoneNumber = playerState.ZoneNumber;
            ...
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in ExecuteList:\n{ex}");
            }
        }
        ...
        private void SendToPlayer(ulong steamId, string message)
        {
            ...
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in SendToPlayer:\n{ex}");
            }
        }
        ...
        private void LoadZonesFromCustomData()
        {
                ...
                foreach (var entity in entities)
                {
                    ...
                    foreach (var slim in blocks)
                    {
                        ...
                        if (_zones.ContainsKey(zoneNumber))
                        {
                            ...
                            _logger?.Warn(MODULE, $"Zone {zoneNumber} doppelt — übersprungen.");
                            continue;
                        }
                        ...
                        {
                            zone.HasError     = true;
                            zone.ErrorMessage = $"Zone {zoneNumber}: Keine Connectors — bitte !pbc autotrans scan ausführen.";
                            
                            _logger?.Warn(MODULE, $"Zone {zoneNumber} ohne Connectors.");
                        }
            ...
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in LoadZonesFromCustomData:\n{ex}");
            }
        }
        ...
        private void ExecuteZoneScan(IMyPlayer player)
        {
            try
            {
                ...
                ScanLog(player, $"  Gefunden: {allContainers.Count} Container, {allConnectors.Count} Connectors, {allZoneLcds.Count} Zone-LCDs, {allSorters.Count} Sortierer");
                ...
                foreach (var container in allContainers)
                {
                    ...
                    if (_zones.ContainsKey(zoneNumber))
                    {
                        ...
                        _zones[zoneNumber] = zone;
                        zonesError++;
                        ScanLog(player, $"  [ERROR] Zone {zoneNumber}: Doppelte ZoneNumber!");
                        continue;
                    }
                    ...
                    if (containerInv == null)
                    {
                        ...
                        _zones[zoneNumber] = zone;
                        zonesError++;
                        ScanLog(player, $"  [ERROR] Zone {zoneNumber}: Container kein Inventar.");
                        continue;
                    }
                    ...
                    if (zone.ConnectorIds.Count == 0)
                    {
                        ...
                        _zones[zoneNumber] = zone;
                        zonesError++;
                        ScanLog(player, $"  [ERROR] Zone {zoneNumber}: Kein Connector gefunden.");
                        continue;
                    }
                    ...
                    _zones[zoneNumber] = zone;
                    zonesOk++;
                    ScanLog(player, $"  [OK] Zone {zoneNumber}: {zone.ConnectorIds.Count} Connector(s), {zone.LcdIds.Count} LCD(s), {zone.SorterInIds.Count} Sortierer-IN, {zone.SorterOutIds.Count} Sortierer-OUT gefunden.");
                }
                ...
                if (_zones.Count == 0)
                    ScanLog(player, "Keine Zonen gefunden — ZoneNumber in Container CustomData setzen.");
                else
            ...
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in ExecuteZoneScan:\n{ex}");
                ScanLog(player, "Scan fehlgeschlagen — siehe Log.");
            }
        }
        ...
        private void WriteZoneBlocksToCustomData(IMyTerminalBlock container, List<long> connectorIds, List<long> lcdIds, List<long> sorterInIds, List<long> sorterOutIds)
        {
            ...
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in WriteZoneBlocksToCustomData:\n{ex}");
            }
        }
        ...
        private void ExecuteReport(IMyPlayer player)
        {
            try
            {
                if (_zones.Count == 0)
                {
                    _commandModule.SendMessage(player, "AutoTransfer Report: Keine Zonen im RAM — bitte !pbc autotrans scan ausführen.");
                    return;
                }
            ...
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in ExecuteReport:\n{ex}");
            }
        }
        ...
        private void ScanAndDeployTemplates()
        {
            ...
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in ScanAndDeployTemplates:\n{ex}");
            }
        }
        ...
        private void OnEntityAdd(IMyEntity entity)
        {
            ...
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in OnEntityAdd:\n{ex}");
            }
        }
        ...
        private void OnBlockAdded(IMySlimBlock slim)
        {
            ...
            {
                MyLog.Default.WriteLineAndConsole($"[PhantombiteAutoTransfer] AutoTransfer ERROR in OnBlockAdded:\n{ex}");
            }
        }
